...
/**
 * Class to represent a comparator for a pair that compairs the first member
 * of that pair
...
public class PairFirstComparator implements Comparator<Pair<? extends Comparable, ?>> {
    ...
    public PairFirstComparator(boolean descent) {
        this.descent = descent;
    }


    public int compare(Pair<? extends Comparable, ?> o1, Pair<? extends Comparable, ?> o2) {
        int value = o1.getFirst().compareTo(o2.getFirst());
        if (descent)
            value = -value;
        return value;
    }
